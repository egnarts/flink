 ...
 * should be written to inside the base directory. The {@code BucketAssigner} can, for example, use
 * time or a property of the element to determine the bucket directory. The default {@code
 * BucketAssigner} is a {@link DateTimeBucketAssigner} which will create one new bucket every hour.
 * You can specify a custom {@code BucketAssigner} using the {@code
 * setBucketAssigner(bucketAssigner)} method, after calling {@link
 * StreamingFileSink#forRowFormat(Path, Encoder)} or {@link StreamingFileSink#forBulkFormat(Path,
 * BulkWriter.Factory)}.
 *
 ...
 * prefix "prefix" and a suffix ".ext", a file named {@code "prefix-1-17.ext"} contains the data
 ...
 * finished}. The reason for this is how the sink works together with the checkpointing mechanism to
 * provide exactly-once semantics and fault-tolerance. The part file that is currently being written
 * to is {@code in-progress}. Once a part file is closed for writing it becomes {@code pending}.
 * When a checkpoint is successful the currently pending files will be moved to {@code finished}.
 *
...
public class StreamingFileSink<IN> extends RichSinkFunction<IN>
    ...
    public abstract static class BucketsBuilder<
                    IN, BucketID, T extends BucketsBuilder<IN, BucketID, T>>
    ...
    public static class RowFormatBuilder<IN, BucketID, T extends RowFormatBuilder<IN, BucketID, T>>
        ...
        public <ID>
                StreamingFileSink.RowFormatBuilder<IN, ID, ? extends RowFormatBuilder<IN, ID, ?>>
    ...
    public static class BulkFormatBuilder<
        ...
        public <ID>
                StreamingFileSink.BulkFormatBuilder<IN, ID, ? extends BulkFormatBuilder<IN, ID, ?>>
